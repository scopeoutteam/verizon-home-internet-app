...
// ── MCP Server Factory ────────────────────────────────────────────────────
// Each session needs its own McpServer instance
function createServer(): McpServer {
  ...
  server.tool(
    ...
    async ({ plan_ids }) => {
      ...
      const comparison = {
        plans: selected,
        fields: [
          "name", "type", "download_mbps", "upload_mbps",
          "price_base", "price_with_mobile", "price_lock_years",
          "data_cap", "includes_router", "includes_whole_home_wifi",
          "gift_card_amount", "perks", "best_for",
        ],
      };
  ...
  server.tool(
    ...
    async ({ plan_id, has_verizon_mobile, selected_perks }) => {
      ...
      const basePrice = has_verizon_mobile ? plan.price_with_mobile : plan.price_base;
      const mobileDiscount = has_verizon_mobile ? plan.price_base - plan.price_with_mobile : 0;

      let perkCosts: { name: string; price: number }[] = [];
      if (selected_perks && selected_perks.length > 0) {
        const freePerkCount = plan.perks.some((p: string) => p.includes("perk on us")) ? 1 : 0;
        selected_perks.forEach((perkName, idx) => {
          const perk = addOnPerks.find((p) => p.name === perkName);
          if (perk) {
            perkCosts.push({ name: perk.name, price: idx < freePerkCount ? 0 : perk.price });
          }
        });
      }
      ...
      return {
        structuredContent: {
          ...
          first_year_savings: mobileDiscount * 12,
          gift_card_amount: plan.gift_card_amount,
        },
        content: [
          {
            type: "text" as const,
            text: `${plan.name}: $${totalMonthly.toFixed(2)}/mo${has_verizon_mobile ? ` (saving $${mobileDiscount.toFixed(2)}/mo with Verizon Mobile)` : ""}. With Auto Pay. Taxes & fees extra. Subject to credit approval.`,
          },
        ],
      };
    }
    ...
    async ({ plan_id, address, zip_code, has_verizon_mobile, selected_perks }) => {
      ...
      if (selected_perks) {
        selected_perks.forEach((perkName, idx) => {
          const perk = addOnPerks.find((p) => p.name === perkName);
          if (perk) {
            const price = idx < freePerkCount ? 0 : perk.price;
      ...
      const matchingPromos = promotions.filter((p) => p.eligible_plans.includes(plan_id));

      return {
        structuredContent: {
          order_summary: {
            plan: plan.name, type: plan.type,
            speed: `${plan.download_mbps} Mbps download${plan.upload_mbps ? ` / ${plan.upload_mbps} Mbps upload` : ""}`,
            address: `${address}, ${zip_code}`, monthly_total: totalMonthly,
            price_breakdown: { base_price: plan.price_base, mobile_discount: has_verizon_mobile ? plan.price_base - plan.price_with_mobile : 0, perks: perkDetails },
            price_lock_years: plan.price_lock_years,
            gift_card: plan.gift_card_amount > 0 ? `$${plan.gift_card_amount} Amazon Gift Card` : null,
            promotions: matchingPromos.map((p) => p.title),
            includes: [plan.includes_router ? "Router included" : null, plan.includes_whole_home_wifi ? "Whole-Home Wi-Fi included" : null, "No annual contract", plan.data_cap === null ? "No data caps" : null].filter(Boolean),
          },
...
async function createSession() {
  ...
  const sid = (transport as any).sessionId;
  if (sid) transports.set(sid, { transport, server });
...
app.get("/mcp", async (req, res) => {
  try {
    ...
    // For SSE connections without a session, create one
